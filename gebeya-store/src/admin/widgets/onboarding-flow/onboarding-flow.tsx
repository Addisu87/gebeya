...
const OnboardingFlow = (props: OnboardingWidgetProps) => {
				...
				case 'nextjs':
					return [
						...
						},
						{
							...
							title:
								'Preview Product in your Next.js Storefront',
						...
						},
						{
							...
							title:
								'Create an Order using your Next.js Storefront',
				...
				default:
					return [
						...
						},
						{
							id: 'setup_finished',
							title:
								'Setup Finished: Start developing with Medusa',
	...
	const isStepComplete = useCallback(
		(step_id: STEP_ID) => {
			return (
				findStepIndex(currentStep) > findStepIndex(step_id)
			);
		},
		...
		async (
			...
			switch (onboardingStep) {
				...
				case 'setup_finished':
					...
					if (orderId) {
						return (
							await client.admin.orders.retrieve(orderId)
						).order;
					}
				...
				case 'preview_product':
					...
					if (productId) {
						return (
							await client.admin.products.retrieve(
								productId,
							)
						).product;
					}
	...
	useEffect(() => {
		const onboardingStep = searchParams.get(
			'onboarding_step',
		) as STEP_ID;
		const onboardingStepIndex =
			findStepIndex(onboardingStep);
		if (
			onboardingStep &&
